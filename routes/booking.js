...
router.post("/flight_booking", isLoggedIn, async (req, res) => {
  ...
  try {
    const flight_query = await db.query(
      "SELECT business_seat, economy_seat, (CAST(departure_date || ' ' || departure_time AS timestamp)) AS departure_timestamp FROM flight_schedule WHERE flight_code = $1",
      [fcode],
    );
    ...
    const depatureTimeStamp = flight_query.rows[0].departure_timestamp;
    const departureDateTime = new Date(`${depatureTimeStamp}`);
    const currentDateTime = new Date();
    const timeDifference = departureDateTime - currentDateTime;
    if (timeDifference < 0) {
      req.session.userInput = req.body;
      console.log("Flight has already departed");
      const error_message = `Flight ${fcode} has already departed`;
    ...
    if (timeDifference < 3 * 60 * 60 * 1000) {
      req.session.userInput = req.body;
      console.log("Flight is departing in less than 3 hours");
      const error_message =
        `Flight ${fcode} is departing in less than 3 hours!! You can't book this flight anymore`;
      req.flash("error", error_message);
      res.redirect("/flight_booking");
      return;
    }
