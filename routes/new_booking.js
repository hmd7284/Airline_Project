...
router.get("/flight_search", isLoggedIn, async (req, res) => {
  try {
    ...
    const transactions = await fetchBookingQueue(
      req.session.userId,
      req,
      transactionid,
    );
...
async function fetchBookingQueue(userId, req, transactionid) {
  try {
    ...
    const result = await client.query(
      "SELECT t.transaction_id, t.booking_date, t.status, t.discount, t.total_amount, o.order_id, o.flight_code, o.type, o.price, o.quantity, o.total FROM transactions t LEFT JOIN transactions_order o ON t.transaction_id = o.transaction_id JOIN flight_schedule fs WHERE fs.flight_code = o.flight_code JOIN route r ON fs.route = r.route_code WHERE t.customer_id = $1 AND t.transaction_id = $2",
...
router.get("/booking_queue", isLoggedIn, async (req, res) => {
  try {
    ...
    const airports = await fetchAirportsFromDatabase();
    res.render("new_user_booking.ejs", { transactions, airports });
  } catch (error) {
    console.error("Error retrieving booking history:", error);
